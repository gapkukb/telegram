...
function filter(validate: Validation, debug: boolean, url: string, payload?: Record<string | number, any>) {
...
export default function normalizePlugin(option: NormalizeOption) {
    const { enable = true, debug = false, validate = defaultValidate } = option || {}
