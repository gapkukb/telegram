...
abstract class Gutter {
  /// equal to { 4px }
  static final xxs = _base * 0.5;

  /// equal to { 8px }
  static final xs = _base * 1;

  /// equal to { 16px }
  static final sm = _base * 2;

  /// equal to { 24px }
  static final md = _base * 3;

  /// equal to { 32px }
  static final lg = _base * 4;

  /// equal to { 40px }
  static final xl = _base * 5;

  /// equal to { 48px }
  static final xxl = _base * 6;

  static final all = _Generate((size) => EdgeInsets.all(size));
  static final horizontal = _Generate((size) => EdgeInsets.symmetric(horizontal: size));
  static final vertical = _Generate((size) => EdgeInsets.symmetric(vertical: size));
  ...
  static final exceptLeft = _Generate((size) => EdgeInsets.fromLTRB(size, 0, 0, 0));
  static final exceptTop = _Generate((size) => EdgeInsets.fromLTRB(0, size, 0, 0));
  static final exceptRight = _Generate((size) => EdgeInsets.fromLTRB(0, 0, size, 0));
  static final exceptBottom = _Generate((size) => EdgeInsets.fromLTRB(0, 0, 0, size));
}
